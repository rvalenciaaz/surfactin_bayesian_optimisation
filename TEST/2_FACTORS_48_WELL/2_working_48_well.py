...
#reference values
ref_values=pd.DataFrame({"sample":["0_REF","0_CTRL"],"Glucose":[80,80],"NH4Cl":[80,80],"extra":[400,400],"water":[160,240]})
...
tem["Glucose"]=tem["sample"].apply(lambda x: re1dict[x])
tem["NH4Cl"]=tem["sample"].apply(lambda x: re2dict[x])
    ...
    for i in pd.unique(tem["sample"]):
        ...
        for compi in mini.columns[3:3+dim]:
            ...
            for res, ving in zip(mini[compi],mini["well_position"]):
                f.write("\tp300.aspirate(80, reservoir['"+res+"'])\n")
                f.write("\tp300.dispense(80, plate['"+ving+"'])\n")
